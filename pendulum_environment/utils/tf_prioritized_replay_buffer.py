...
class TFPrioritizedReplayBuffer(replay_buffer.ReplayBuffer):
    ...
    def variables(self):
        return (
            self._data_table.variables() + self._id_table.variables() + [self._last_id]
        )
    ...
    def _add_batch(self, items):
        ...
        with tf.device(self._device), tf.name_scope(self._scope):
            ...
            write_prio_op = self._prio_table.write(
                write_rows, tf.fill(tf.size(write_rows), self._max_prio)
            )
        ...
        with tf.device(self._device), tf.name_scope(self._scope):
            with tf.name_scope("get_next"):
                ...
                with tf.control_dependencies([assert_nonempty]):
                    ...
                    batch_offsets = (
                        tf.range(0, self._batch_size, dtype=tf.int64) * self._max_length
                    )
                    batch_ids = tf.range(min_val, max_val, dtype=tf.int64)
                    # to each batch_offset, add batch_ids
                    prio_all_ids = tf.map_fn(
                        fn=lambda t: tf.add(t, batch_ids), elems=batch_offsets
                    )
                    prio_all_ids = tf.reshape(prio_all_ids, [-1])
                    prio_all_ids = tf.math.mod(
                        prio_all_ids, self._capacity
                    )  # because batch-ids are not capped, we need to do a mod here

                    probabilities = self._prio_table.read(prio_all_ids)
                    log_probabilities = tf.math.log([probabilities])

                    ids_of_sampled_prio_ids = tf.random.categorical(
                        log_probabilities, rows_shape[0]
                    )
                    chosen_element_ids = tf.gather(
                        prio_all_ids, ids_of_sampled_prio_ids
                    )
                    chosen_element_ids = tf.reshape(chosen_element_ids, [-1])
                    chosen_element_probabilities = tf.gather(
                        probabilities, ids_of_sampled_prio_ids
                    )
                    chosen_element_probabilities = tf.reshape(
                        chosen_element_probabilities, [-1]
                    )
                ...
                if num_steps is None:
                    ...
                    if time_stacked:
                        ...
                        if sample_batch_size:
                            ...
                            chosen_element_ids_expanded = tf.tile(
                                tf.expand_dims(chosen_element_ids, -1), [1, num_steps]
                            )
                        else:
                            step_range = tf.reshape(step_range, [num_steps])

                        rows_to_get = tf.math.mod(
                            step_range + chosen_element_ids_expanded, self._capacity
                        )
                        data = self._data_table.read(rows_to_get)
                        data_ids = self._id_table.read(rows_to_get)
                    else:
                        ...
                        for step in range(num_steps):
                            steps_to_get = tf.math.mod(
                                chosen_element_ids + step, self._capacity
                            )
                ...
                buffer_info = BufferInfo(
                    ids=chosen_element_ids,  # this was returning the batch ID of each element, now we return actual ids to allow updates
    ...
    def _single_deterministic_pass_dataset(
        ...
        if (
            ...
            raise ValueError(
                ...
                "this case, ALL data will be dropped by the deterministic dataset.".format(
                    static_size, static_self_batch_size
                )
            )
        if (
            self._dataset_drop_remainder
            ...
            raise ValueError(
                ...
                "dataset_drop_remainder is True.  In "
                "this case, ALL data will be dropped by the deterministic dataset.".format(
                    static_num_steps, static_self_max_length
                )
            )
    ...
    def _clear(
        self, clear_all_variables=False
    ):  # TODO may need to clear weights as well
